...
void testAstlAdd(FunctionOpInterface funcOp) {
  // clang-format off
...
void testInterfaces(FunctionOpInterface funcOp) {
  ...
  auto matcher =
    StructuredOpMatcher::make<linalg::GenericOp>()
      .operation(
        VerifyOpProperty(OpTrait::astl::checkUnitStrideInnerLoop));
  // clang-format on
...
void testAstlIdentity(FunctionOpInterface funcOp) {
  // clang-format off
  SmallVector<Value> operands;
  auto matcher = 
    StructuredOpMatcher::make<linalg::GenericOp>()
      ...
      .operation(VerifyOpProperty(OpTrait::astl::checkUnitStrideInnerLoop))
...
void testAstlRank(FunctionOpInterface funcOp) {
  // clang-format off
  auto matcherRank1 =
    StructuredOpMatcher::make<linalg::GenericOp>()
...
void TestStructuralMatchers::runOnOperation() {
  ...
  if (f.getName() == "test_vnni_brgemm")
    testVnniBrgemm(f);
  if (f.getName() == "test_astl_add")
    testAstlAdd(f);
  if (f.getName() == "astl_add_must_not_match")
    testAstlAdd(f);
  if (f.getName() == "test_predicates")
    testPredicates(f);
  if (f.getName() == "test_interfaces")
    testInterfaces(f);
  if (f.getName() == "test_astl_identity")
    testAstlIdentity(f);
