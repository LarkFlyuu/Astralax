...
struct LinalgDeGeneralize : LinalgDeGeneralizeBase<LinalgDeGeneralize> {
  void runOnOperation() override {
    ...
    astl::populateAstlDeGeneralizationPatterns(patterns);
...
// From linalg.generic to ASTL brgemm (VNNI).
struct AstlBrgemmDeGeneralizationPattern
    : public OpRewritePattern<linalg::GenericOp> {
  using OpRewritePattern<linalg::GenericOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(linalg::GenericOp linalgOp,
                                PatternRewriter &rewriter) const override {
    if (!astl::utils::isAstlVnniOp(linalgOp, /*captures=*/nullptr))
      return failure();
    ...
    rewriter.replaceOpWithNewOp<astl::BrgemmOp>(linalgOp, operands,
...
void mlir::astl::populateAstlDeGeneralizationPatterns(
    RewritePatternSet &patterns) {
  patterns.add<AstlBrgemmDeGeneralizationPattern>(patterns.getContext());
}
