...
struct RewriteConv2DNhwcHwcfToMatmul : OpRewritePattern<linalg::GenericOp> {
  ...
  LogicalResult matchAndRewrite(linalg::GenericOp genericOp,
    ...
    if (!astl::utils::isMarkedWithAstl(genericOp, "astl.Conv2DNhwcHwcfOp"))
      return failure();
...
// Interchange iterators for a astl.Conv2DNhwcHwcfOp.
...
// with astl.Conv2DNhwcHwcfOp such that later pattern can pick it up.
struct GeneralizeConv2DNhwcHwcf : OpRewritePattern<linalg::Conv2DNhwcHwcfOp> {
  ...
  LogicalResult matchAndRewrite(linalg::Conv2DNhwcHwcfOp convOp,
    ...
    generic.setLibraryCallAttr(rewriter.getStringAttr("astl.Conv2DNhwcHwcfOp"));
    return success();
  }
};

// Block a Conv2DNchwFchw. The pattern returns a generic operation
// marked as 'astl.blocked.Conv2DNchwFchwOp' on success.
struct BlockConv2DNchwFchw : OpRewritePattern<linalg::Conv2DNchwFchwOp> {
  ...
  LogicalResult matchAndRewrite(linalg::Conv2DNchwFchwOp convOp,
    ...
    generic.setLibraryCallAttr(
        rewriter.getStringAttr("astl.BlockedConv2DNchwFchwOp"));
    return success();
  }
...
struct CollapseFilterAndImage : OpRewritePattern<linalg::GenericOp> {
  ...
  LogicalResult collapseFilterPreconditions(linalg::GenericOp linalgOp) const {
    if (!astl::utils::isMarkedWithAstl(linalgOp, "astl.BlockedConv2DNchwFchwOp"))
      return failure();
  ...
  LogicalResult matchAndRewrite(linalg::GenericOp linalgOp,
    ...
    linalg::GenericOp replacementOp = rewriter.create<linalg::GenericOp>(
        ...
        /*library_call=*/"astl.BlockedandCollapsedConv2DNchwFchwOp");
...
struct InterchangeAfterBlockingAndCollapsing
  ...
  LogicalResult matchAndRewrite(linalg::GenericOp linalgOp,
                                PatternRewriter &rewriter) const override {
    if (!astl::utils::isMarkedWithAstl(linalgOp,
                                     "astl.BlockedandCollapsedConv2DNchwFchwOp"))
      return failure();
    ...
    if (failed(maybeInterchange))
      return failure();
    StringAttr name =
        rewriter.getStringAttr("astl.BlockedCollapsedAndInterConv2DNchwFchwOp");
...
struct MapToBRGEMM : OpRewritePattern<linalg::GenericOp> {
  ...
  LogicalResult matchAndRewrite(linalg::GenericOp linalgOp,
                                PatternRewriter &rewriter) const override {
    if (!astl::utils::isMarkedWithAstl(
            linalgOp, "astl.BlockedCollapsedAndInterConv2DNchwFchwOp"))
      return failure();
...
mlir::astl::createRewriteConvToMatmulOrBrgemmPass() {
