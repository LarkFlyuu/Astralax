...
// Convert astl.add to SCF loops.
struct ConvertAstlAddOp : public OpRewritePattern<AddOp> {
  using OpRewritePattern<AddOp>::OpRewritePattern;

  ConvertAstlAddOp(MLIRContext *ctx, bool parallel)
  ...
  LogicalResult matchAndRewrite(AddOp addOp,
    ...
    if (!addOp.hasBufferSemantics())
      return rewriter.notifyMatchFailure(
          addOp, "astl loop lowering expects memref type");
...
void buildUnaryLoop(
    PatternRewriter &rewriter, AstlOp astlOp,
    function_ref<void(OpBuilder &, Location, ValueRange)> bodyBuilder,
    bool parallel = false) {
  assert(astlOp.isUnary() && "Expect astl unary op");

  Location loc = astlOp.getLoc();

  SmallVector<Value> ubs;
  size_t rank = astlOp.getOutput().getType().cast<MemRefType>().getRank();
  ArrayRef<int64_t> shapeOutput =
      astlOp.getOutput().getType().cast<MemRefType>().getShape();
  for (size_t idx = 0; idx < rank; idx++) {
...
// Converts astl.identity to SCF loops.
struct ConvertAstlIdentityOp : public OpRewritePattern<IdentityOp> {
  using OpRewritePattern<IdentityOp>::OpRewritePattern;

  ConvertAstlIdentityOp(MLIRContext *ctx, bool parallel)
      : OpRewritePattern(ctx), parallel(parallel) {}
  ...
  LogicalResult matchAndRewrite(IdentityOp identityOp,
    ...
    if (!identityOp.hasBufferSemantics())
      return rewriter.notifyMatchFailure(
          identityOp, "astl loop lowering expects memref type");
...
// Convert astl.relu to SCF loops.
struct ConvertAstlReluOp : public OpRewritePattern<ReluOp> {
  using OpRewritePattern<ReluOp>::OpRewritePattern;

  ConvertAstlReluOp(MLIRContext *ctx, bool parallel)
      : OpRewritePattern(ctx), parallel(parallel) {}
  ...
  LogicalResult matchAndRewrite(ReluOp reluOp,
    ...
    if (!reluOp.hasBufferSemantics())
      return rewriter.notifyMatchFailure(
          reluOp, "astl loop lowering expects memref type");
...
// Convert astl.zero to SCF loops.
struct ConvertAstlZeroOp : public OpRewritePattern<ZeroOp> {
  using OpRewritePattern<ZeroOp>::OpRewritePattern;

  ConvertAstlZeroOp(MLIRContext *ctx, bool parallel)
      : OpRewritePattern(ctx), parallel(parallel) {}
  ...
  LogicalResult matchAndRewrite(ZeroOp zeroOp,
    ...
    if (!zeroOp.hasBufferSemantics())
      return rewriter.notifyMatchFailure(
          zeroOp, "astl loop lowering expects memref type");
...
// Convert astl.gemm to SCF loops.
struct ConvertAstlGemmOp : public OpRewritePattern<GemmOp> {
  using OpRewritePattern<GemmOp>::OpRewritePattern;

  ConvertAstlGemmOp(MLIRContext *ctx, bool parallel)
      : OpRewritePattern(ctx), parallel(parallel) {}
  ...
  LogicalResult matchAndRewrite(GemmOp matmulOp,
    ...
    if (!matmulOp.hasBufferSemantics())
      return rewriter.notifyMatchFailure(
          matmulOp, "astl loop lowering expects memref type");
...
// Convert astl.brgemm to SCF loops.
struct ConvertAstlBrgemmOp : public OpRewritePattern<BrgemmOp> {
  using OpRewritePattern<BrgemmOp>::OpRewritePattern;

  ConvertAstlBrgemmOp(MLIRContext *ctx, bool parallel)
      : OpRewritePattern(ctx), parallel(parallel) {}
  ...
  LogicalResult matchAndRewrite(BrgemmOp brgemmOp,
    ...
    if (!brgemmOp.hasBufferSemantics())
      return rewriter.notifyMatchFailure(
          brgemmOp, "astl loop lowering expects memref type");
...
// Convert astl.fused_brgemm to SCF loops.
struct ConvertAstlFusedBrgemmOp : public OpRewritePattern<FusedBrgemmOp> {
  ...
  LogicalResult matchAndRewrite(FusedBrgemmOp fusedBrgemmOp,
    ...
    if (!fusedBrgemmOp.hasBufferSemantics())
      return rewriter.notifyMatchFailure(
          fusedBrgemmOp, "astl loop lowering expects memref type");

    return astl::utils::splitAndReplaceFusedOp(fusedBrgemmOp, rewriter);
  }
};

void populateAstlToLoopsPatterns(RewritePatternSet &patterns, bool parallel) {
  // clang-format off
  patterns.add<ConvertAstlAddOp, 
               ConvertAstlIdentityOp,
               ConvertAstlGemmOp,
               ConvertAstlBrgemmOp,
               ConvertAstlFusedBrgemmOp,
               ConvertAstlReluOp,
               ConvertAstlZeroOp>(patterns.getContext(), parallel);
  // clang-format on
}

struct ConvertAstlToLoops : public ConvertAstlToLoopsBase<ConvertAstlToLoops> {
  ConvertAstlToLoops() = default;
  ConvertAstlToLoops(bool parallel) { this->parallel = parallel; }

  void runOnOperation() override {
    RewritePatternSet patterns(&getContext());
    populateAstlToLoopsPatterns(patterns, parallel);
...
mlir::astl::createConvertAstlToLoopsPass(bool parallel) {
  return std::make_unique<ConvertAstlToLoops>(parallel);
}
