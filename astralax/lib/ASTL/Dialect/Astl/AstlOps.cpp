//===- AstlOps.cpp - astl dialect ops ----------------------------*- C++ -*-===//
//
...
static ParseResult parseAstlOp(OpAsmParser &parser, OperationState &result) {
...
// Print a astl op. Note that `out` can be null. It is null for unary and binary
// at tensor abstraction.
static void printAstlOp(OpAsmPrinter &printer, ValueRange operands,
...
static void astlOpBuilderMemRef(OpBuilder &builder, OperationState &state,
...
static void astlOpBuilderTensor(OpBuilder &builder, OperationState &state,
...
static void getEffectsImpl(
    ...
    SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>
        &effects) {
  if (astlOp.hasTensorSemantics())
    return;
  for (auto operand : astlOp.getInputs()) {
    if (!operand.getType().isa<MemRefType>())
      continue;
    effects.emplace_back(MemoryEffects::Read::get(), operand,
                         SideEffects::DefaultResource::get());
  }
  effects.emplace_back(MemoryEffects::Write::get(), astlOp.getOutput(),
                       SideEffects::DefaultResource::get());
}
...
void IdentityOp::build(OpBuilder &builder, OperationState &state, Value input,
                       Value output) {
  astlOpBuilderMemRef(builder, state, input, output);
}

void IdentityOp::build(OpBuilder &builder, OperationState &state, Value input,
                       Type outputType) {
  astlOpBuilderTensor(builder, state, input, outputType);
}

void IdentityOp::print(OpAsmPrinter &printer) {
  printAstlOp(printer, getInputs(), getOutputs(), getResultTypes(), *this);
}

ParseResult IdentityOp::parse(OpAsmParser &parser, OperationState &result) {
  return parseAstlOp(parser, result);
}
...
void AddOp::build(OpBuilder &builder, OperationState &state, ValueRange inputs,
                  Value output) {
  astlOpBuilderMemRef(builder, state, inputs, output);
}

// Builder for tensor abstraction.
void AddOp::build(OpBuilder &builder, OperationState &state, ValueRange inputs,
                  Type outputType) {
  astlOpBuilderTensor(builder, state, inputs, outputType);
}
...
static LogicalResult verifyGemmLikeOperands(OpTy operation) {
  ...
  // Validate operand A.
  bool isGemmOp = isa<astl::GemmOp>(operation.getOperation());
...
static void FusedBrgemmOpBuilder(OpBuilder &builder, OperationState &state,
  ...
  if (output)
    astlOpBuilderMemRef(builder, state, allInputs, output);
  else
    astlOpBuilderTensor(builder, state, allInputs, outputType);
...
void FusedBrgemmOp::print(OpAsmPrinter &printer) {
  printer << " [" << UNARY << " = "
          << astl::stringifyFusedUnaryOpKind(getUnaryKind()) << ", " << BINARY
          << " = " << astl::stringifyFusedBinaryOpKind(getBinaryKind()) << "]";
  printAstlOp(printer, getInputs(), getOutputs(), getResultTypes(), *this);
}
