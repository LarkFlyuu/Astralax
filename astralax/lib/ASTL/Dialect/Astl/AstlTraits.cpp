...
using namespace mlir::OpTrait::astl;
...
LogicalResult
mlir::OpTrait::astl::verifyBroadcastableShape(Operation *op,
                                             bool emitDiagnostic) {
  TypeRange operandTypes = op->getOperandTypes();

  // In astl at memref the last operand is the output buffer, while in astl on
...
// TODO: refactor these interfaces to use the astl op interface. Note
// that we use the `check*` on linalg operations.
// Verify all the operands have stride one in the fastest-varying dimension.
LogicalResult
mlir::OpTrait::astl::verifyUnitStrideInnerLoop(Operation *op,
                                              bool emitDiagnostic) {
  if (hasTensorSemantics(op))
    return success();
...
LogicalResult mlir::OpTrait::astl::checkBroadcastableShape(Operation *op) {
  return verifyBroadcastableShape(op, /*emitDiagnostic=*/false);
}

LogicalResult mlir::OpTrait::astl::checkUnitStrideInnerLoop(Operation *op) {
...
LogicalResult mlir::OpTrait::astl::verifyArity(Operation *op,
                                              unsigned numInput) {
  assert(op->template hasTrait<OpTrait::AttrSizedOperandSegments>());
  assert(isa<mlir::astl::AstlOp>(op) && "expect a astl operation");
  auto astlOp = cast<mlir::astl::AstlOp>(op);
  if (astlOp.hasTensorSemantics())
    return verifyArityImpl(astlOp, numInput, /*numberOfOutput=*/0);
  return verifyArityImpl(astlOp, numInput, /*numberOfOutput=*/1);
}
