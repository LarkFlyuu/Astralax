...
#define DEBUG_TYPE "astl-bufferize"
...
static bool canBufferizeOnOperandImpl(Operation *op, Value operand,
  ...
  if (isConstantVal(operand) || !isInSameRepetitveRegion(op, operand, state))
    return false;
  return astlOp.getResultType() == operand.getType();
}
...
static bool bufferizesToAllocationUnaryImpl(Operation *op, Value value) {
  auto unaryOp = cast<astl::AstlOp>(op);
...
struct ReluBufferizationInterface
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
    ...
    return bufferizeUnaryOp<astl::ReluOp>(op, rewriter, options);
  }
...
struct IdentityBufferizationInterface
    : public BufferizableOpInterface::ExternalModel<
          IdentityBufferizationInterface, astl::IdentityOp> {
  bool bufferizesToMemoryRead(Operation *op, OpOperand &opOperand,
                              const AnalysisState &state) const {
    return bufferizesToMemoryReadUnaryImpl(op, opOperand, state);
  }
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
                          const BufferizationOptions &options) const {
    return bufferizeUnaryOp<astl::IdentityOp>(op, rewriter, options);
  }
...
struct ZeroBufferizationInterface
  ...
  bool bufferizesToMemoryRead(Operation *op, OpOperand &opOperand,
                              const AnalysisState &state) const {
    // astl.zero has only write effects.
    return false;
  }
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
                          const BufferizationOptions &options) const {
    return bufferizeUnaryOp<astl::ZeroOp>(op, rewriter, options);
  }

  bool bufferizesToAllocation(Operation *op, Value value) const {
    // astl.zero is by construction always in place.
    return false;
  }
};
...
static bool bufferizesToAllocationBinaryImpl(Operation *op, Value value) {
  auto binaryOp = cast<astl::AstlOp>(op);
...
struct AddBufferizationInterface
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
                          const BufferizationOptions &options) const {
    return bufferizeBinaryOp<astl::AddOp>(op, rewriter, options);
  }
...
struct GemmBufferizationInterface
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
                          const BufferizationOptions &options) const {
    return bufferizeTernaryOp<astl::GemmOp>(op, rewriter, options);
  }
};

struct BrgemmBufferizationInterface
    : public BufferizableOpInterface::ExternalModel<
          BrgemmBufferizationInterface, astl::BrgemmOp> {
  bool bufferizesToMemoryRead(Operation *op, OpOperand &opOperand,
                              const AnalysisState &state) const {
    return bufferizesToMemoryReadTernaryImpl(op, opOperand, state);
  }
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
                          const BufferizationOptions &options) const {
    return bufferizeTernaryOp<astl::BrgemmOp>(op, rewriter, options);
  }
};

struct FusedBrgemmBufferizationInterface
    : public BufferizableOpInterface::ExternalModel<
          FusedBrgemmBufferizationInterface, astl::FusedBrgemmOp> {
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
                          const BufferizationOptions &options) const {
    return bufferizeQuaternaryOp<astl::FusedBrgemmOp>(op, rewriter, options);
  }
};
...
void registerBufferizableOpInterfaceExternalModels(DialectRegistry &registry) {
  registry.addExtension(+[](MLIRContext *ctx, astl::AstlDialect *dialect) {
    IdentityOp::attachInterface<astl::IdentityBufferizationInterface>(*ctx);
    ReluOp::attachInterface<astl::ReluBufferizationInterface>(*ctx);
    ZeroOp::attachInterface<astl::ZeroBufferizationInterface>(*ctx);
    AddOp::attachInterface<astl::AddBufferizationInterface>(*ctx);
    GemmOp::attachInterface<astl::GemmBufferizationInterface>(*ctx);
    BrgemmOp::attachInterface<astl::BrgemmBufferizationInterface>(*ctx);
    FusedBrgemmOp::attachInterface<astl::FusedBrgemmBufferizationInterface>(
        *ctx);
  });
}
} // namespace astl
