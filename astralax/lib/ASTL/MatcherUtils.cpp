...
static bool isAstlOp(linalg::LinalgOp linalgOp) {
  // clang-format off
  auto astlMatcher =
  ...
  // clang-format on
  return astlMatcher.match(linalgOp);
}

static bool isAstlBinaryOp(linalg::LinalgOp linalgOp) {
  // clang-format off
  auto binaryMatcher =
      StructuredOpMatcher::make<linalg::LinalgOp>()
          ...
          // TODO: This should not depend on ASTLs.
          .operation(VerifyOpProperty(OpTrait::astl::checkBroadcastableShape));
  // clang-format on
  return isAstlOp(linalgOp) && binaryMatcher.match(linalgOp);
}

static bool isAstlUnaryOp(linalg::LinalgOp linalgOp) {
  // clang-format off
  auto unaryMatcher =
      StructuredOpMatcher::make<linalg::LinalgOp>()
          .operation(NumDpsInits(EqualsTo(1)))
          ...
          .operation(NumOfLoops(EqualsTo(2)));
  // clang-format on
  return isAstlOp(linalgOp) && unaryMatcher.match(linalgOp);
}

// Return true if the linalg.generic can be mapped to a astl.add.
bool isTwoDAddOp(linalg::LinalgOp linalgOp, SmallVectorImpl<Value> *operands) {
  ...
  // clang-format on
  return isAstlBinaryOp(linalgOp) && addMatcher.match(linalgOp);
}
...
// Return true if the linalg.generic can be mapped to a astl.relu.
bool isTwoDReluOp(linalg::LinalgOp linalgOp, SmallVectorImpl<Value> *operands) {
  ...
  auto reluMatcher =
    ...
    // TODO: This should not depend from ASTLs.
    .operation(VerifyOpProperty(OpTrait::astl::checkBroadcastableShape));
  // clang-format on
  return isAstlUnaryOp(linalgOp) && reluMatcher.match(linalgOp);
}

// Return true if the linalg.generic can be mapped to a astl.identity.
bool isTwoDIdentityOp(linalg::LinalgOp linalgOp,
  ...
  if (!isAstlUnaryOp(linalgOp) || !identityMatcher.match(linalgOp) ||
...
// Return true if the linalg.generic can be mapped to a astl.zero.
bool isTwoDZeroOp(linalg::LinalgOp linalgOp, SmallVectorImpl<Value> *operands) {
  ...
  if (!isAstlUnaryOp(linalgOp) || !zeroMatcher.match(linalgOp))
    return false;
  ...
  // Only take the output as astl.zero is an in-place operation.
...
// Return true if the linalg.generic can be mapped to a astl.add + astl.max.
// FIXME: This is necessary because IREE fuses addf + maxf and we don't match
// TODO: This will be done at astl.group level later on
bool isTwoDBiasReluOp(linalg::LinalgOp linalgOp,
  ...
  if (!isAstlBinaryOp(linalgOp) || !biasReluMatcher.match(linalgOp))
    return false;

  // Only take the output as astl.add + astl.relu should be in-place operations.
  auto output = linalgOp.getDpsInitOperands()[0]->get();
  if (!output.getType().isa<ShapedType>())
    return false;
...
bool isTwoDTransposeOp(linalg::LinalgOp linalgOp,
  ...
  // clang-format on 
  if (!isAstlUnaryOp(linalgOp) || !isTwoDTransposeMatcher.match(linalgOp))
    return false;
  if (operands) {
...
bool isTwoDFillOpWithZeros(linalg::LinalgOp linalgOp, SmallVectorImpl<Value> *operands) {
  ...
  // clang-format on
  if (!isAstlUnaryOp(linalgOp) || !isTwoDFillOpWithZerosMatcher.match(linalgOp))
    return false;
  if (operands) {
    operands->push_back(linalgOp.getDpsInputOperands()[0]->get());
    operands->push_back(linalgOp.getDpsInitOperands()[0]->get());
