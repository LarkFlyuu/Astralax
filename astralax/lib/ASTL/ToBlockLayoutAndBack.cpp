...
mlir::linalgx::packVNNIBRGemmOp(RewriterBase &rewriter,
  ...
  auto replacementOp = rewriter.create<astl::BrgemmOp>(
      loc,
...
struct VNNIOnBRGemm : public OpRewritePattern<linalg::BatchReduceMatmulOp> {
  ...
  LogicalResult matchAndRewrite(linalg::BatchReduceMatmulOp brgemmOp,
                                PatternRewriter &rewriter) const override {
    FailureOr<astl::BrgemmOp> packedBRGemm =
...
struct PropagatePackUnPack
  ...
  void runOnOperation() override {
    MLIRContext *ctx = getOperation().getContext();
    RewritePatternSet patterns(ctx);
    astl::populateSinkPackPatterns(patterns);
    (void)applyPatternsAndFoldGreedily(getOperation(), std::move(patterns));
  }
};
...
void mlir::astl::populateSinkPackPatterns(RewritePatternSet &patterns) {
...
mlir::astl::createPackMatmulPass(ArrayRef<int64_t> blockingFactors) {
  return std::make_unique<PackMatmul>(blockingFactors);
}

std::unique_ptr<OperationPass<func::FuncOp>>
mlir::astl::createPackConv2DNchwFchwPass(ArrayRef<int64_t> blockingFactors) {
  return std::make_unique<PackConv2DNchwFchw>(blockingFactors);
}

std::unique_ptr<OperationPass<func::FuncOp>>
mlir::astl::createPackConv2DNhwcHwcfPass(ArrayRef<int64_t> blockingFactors) {
  return std::make_unique<PackConv2DNhwcHwcf>(blockingFactors);
}

std::unique_ptr<OperationPass<func::FuncOp>> mlir::astl::createPackVNNIPass() {
  return std::make_unique<PackVNNI>();
}

std::unique_ptr<OperationPass<func::FuncOp>>
mlir::astl::createPropagatePackUnPackPass() {
  return std::make_unique<PropagatePackUnPack>();
}

std::unique_ptr<OperationPass<func::FuncOp>>
mlir::astl::createSimplifyAndCanonicalizePackPass() {
