// RUN: astl-opt %s -split-input-file -convert-linalg-to-astl | FileCheck %s
...
// CHECK: %{{.+}} = astl.add (%[[ARG0]] : tensor<5x5xf32>, %[[ARG1]] : tensor<5x5xf32>) -> (tensor<5x5xf32>)

// -----

// Expect not to map. ASTL ops require rank 2 on output.
...
// Scalar operands we don't expect any mapping to astl.
#map = affine_map<() -> ()>
...
// CHECK: astl.relu (%[[ARG0]] : tensor<10x10xf32>) -> (tensor<10x10xf32>)
...
// CHECK: astl.relu (%[[ARG1]] : tensor<10x10xf32>) -> (tensor<10x10xf32>)
...
// CHECK: %{{.+}} = astl.identity (%[[ARG0]] : f32) -> (tensor<8x32xf32>)
...
// CHECK: %{{.+}} = astl.identity (%[[ARG0]] : tensor<1x1xf32>) -> (tensor<8x32xf32>)
...
// CHECK: %{{.+}} = astl.identity (%[[ARG1]] : tensor<1x32xf32>) -> (tensor<8x32xf32>)
...
func.func @broadcast_row_identity(%arg0: tensor<8x32xf32>, %arg1: tensor<32xf32>) -> tensor<8x32xf32> {
  // CHECK: astl.identity (%[[ARG1]] : tensor<32xf32>) -> (tensor<8x32xf32>)
  %0 = linalg.generic {
    indexing_maps = [#map1, #map],
    iterator_types = ["parallel", "parallel"]}
...
// CHECK: %{{.+}} = astl.identity (%[[ARG1]] : tensor<8x1xf32>) -> (tensor<8x32xf32>)
...
// This should be fixed, as this op is a valid astl.identity.
