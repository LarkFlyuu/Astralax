// RUN: astl-opt %s | astl-opt | FileCheck %s

// CHECK-LABEL: @astl_dialect
func.func @astl_dialect(%arg0: memref<2x2xf32>,
                       %arg1: memref<2x2xf32>,
                       %arg2: memref<2x2xf32>, %arg3: f32, %arg4: f32,
                       %arg5: memref<1x2xf32>, %arg6: memref<1x2xf32>) {
  // CHECK: astl.add
  astl.add ins(%arg0: memref<2x2xf32>, %arg0: memref<2x2xf32>) outs(%arg2: memref<2x2xf32>)

  // CHECK: astl.identity
  astl.identity ins(%arg0: memref<2x2xf32>) outs(%arg2: memref<2x2xf32>)

  // CHECK: astl.identity
  astl.identity ins(%arg3: f32) outs(%arg2: memref<2x2xf32>)

  // CHECK: astl.relu
  astl.relu ins(%arg0: memref<2x2xf32>) outs(%arg0: memref<2x2xf32>)

  // CHECK: astl.relu
  astl.relu ins(%arg5: memref<1x2xf32>) outs(%arg6: memref<1x2xf32>)

  // CHECK: astl.add
  astl.add ins(%arg5: memref<1x2xf32>, %arg5: memref<1x2xf32>) outs(%arg6: memref<1x2xf32>)

  // CHECK: astl.gemm
  astl.gemm ins(%arg0: memref<2x2xf32>, %arg1: memref<2x2xf32>, %arg2: memref<2x2xf32>)
           outs(%arg2: memref<2x2xf32>)

  // CHECK: astl.zero
  astl.zero ins(%arg1: memref<2x2xf32>) outs(%arg1: memref<2x2xf32>)

  // CHECK: astl.zero {{.+}} {myAttr = "myattr"}
  astl.zero ins(%arg1: memref<2x2xf32>) outs(%arg1: memref<2x2xf32>) {myAttr = "myattr"} 

  return
}

// CHECK-LABEL: func.func @identity_bcast_row
func.func @identity_bcast_row(%arg0: memref<5x1xf32>, %arg1: memref<5x6xf32>) {
  // CHECK: astl.identity
  astl.identity ins(%arg0: memref<5x1xf32>) outs(%arg1: memref<5x6xf32>)
  return
}

// CHECK-LABEL: func.func @identity_bcast_col
func.func @identity_bcast_col(%arg0: memref<5xf32>, %arg1: memref<6x5xf32>) {
  // CHECK: astl.identity
  astl.identity ins(%arg0: memref<5xf32>) outs(%arg1: memref<6x5xf32>)
  return
}

// CHECK-LABEL: func.func @test_brgemm
func.func @test_brgemm(%arg0: memref<2x5x4xf32>, %arg1: memref<2x4x5xf32>,
                      %arg2: memref<5x5xf32>) -> memref<5x5xf32> {
  // CHECK: astl.brgemm
  astl.brgemm ins(%arg0: memref<2x5x4xf32>, %arg1: memref<2x4x5xf32>, %arg2: memref<5x5xf32>)
             outs(%arg2: memref<5x5xf32>)
  return %arg2: memref<5x5xf32>
}

// CHECK-LABEL: func.func @test_gemm_with_Bf16
func.func @test_gemm_with_Bf16(%arg0: memref<6x10xbf16>, %arg1: memref<5x6x2xbf16>,
                               %arg2: memref<6x6xbf16>) -> memref<6x6xbf16> {
  // CHECK: astl.gemm
  astl.gemm ins(%arg0: memref<6x10xbf16>, 
              %arg1: memref<5x6x2xbf16>, %arg2: memref<6x6xbf16>) outs(%arg2: memref<6x6xbf16>)
  return %arg2: memref<6x6xbf16>
}

// CHECK-LABEL: func.func @test_brgemm_with_Bf16
func.func @test_brgemm_with_Bf16(%arg0: memref<64x4x4xbf16>, %arg1: memref<64x2x4x2xbf16>,
                              %arg2: memref<4x4xbf16>) -> memref<4x4xbf16> {
  // CHECK: astl.brgemm
  astl.brgemm ins(%arg0: memref<64x4x4xbf16>, %arg1: memref<64x2x4x2xbf16>, 
                 %arg2: memref<4x4xbf16>) outs(%arg2: memref<4x4xbf16>)
  return %arg2: memref<4x4xbf16>
}

// CHECK-LABEL: func.func @add_bcast_row_operand_one
func.func @add_bcast_row_operand_one(%arg0: memref<5xf32>, %arg1: memref<6x5xf32>, 
                                     %arg2: memref<1x5xf32> ) {
  // CHECK: astl.add
  astl.add ins(%arg0: memref<5xf32>, %arg1: memref<6x5xf32>) outs(%arg1: memref<6x5xf32>)
  // CHECK-NEXT: astl.add
  astl.add ins(%arg1: memref<6x5xf32>, %arg0: memref<5xf32>) outs(%arg1: memref<6x5xf32>)
  // CHECK-NEXT: astl.add
  astl.add ins(%arg0: memref<5xf32>, %arg2: memref<1x5xf32>) outs(%arg1: memref<6x5xf32>)
  return
}

// CHECK-LABEL: func.func @add_bcast_col_operand_one
func.func @add_bcast_col_operand_one(%arg0: memref<6x1xf32>, %arg1: memref<6x5xf32>) {
  // CHECK: astl.add
  astl.add ins(%arg0: memref<6x1xf32>, %arg1: memref<6x5xf32>) outs(%arg1: memref<6x5xf32>)
  // CHECK-NEXT: astl.add
  astl.add ins(%arg1: memref<6x5xf32>, %arg0: memref<6x1xf32>) outs(%arg1: memref<6x5xf32>)
  return
}

// CHECK-LABEL: test_gemm
func.func @test_gemm(%arg0: memref<2x2xf32>) {
  // CHECK: astl.gemm
  astl.gemm ins(%arg0: memref<2x2xf32>, %arg0: memref<2x2xf32>, %arg0: memref<2x2xf32>) 
             outs(%arg0: memref<2x2xf32>)
  return
}

// CHECK-LABEL: fused_brgemm
func.func @fused_brgemm(%arg0: memref<3x32x32xf32>, %arg1: memref<3x32x32xf32>, %arg2: memref<32x32xf32>,
                        %arg3: memref<32x32xf32>) {
  // CHECK: astl.fused_brgemm
  astl.fused_brgemm [unary = relu, binary = add] 
                   ins(%arg0: memref<3x32x32xf32>, %arg1: memref<3x32x32xf32>, 
                       %arg2: memref<32x32xf32>, %arg3: memref<32x32xf32>) outs(%arg3: memref<32x32xf32>)
  return
}
