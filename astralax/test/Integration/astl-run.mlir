// Print mlir
// RUN: astl-run %s -e entry -entry-point-result=void -print-mlir=early 2>&1 | FileCheck %s --check-prefix=EARLY
// RUN: astl-run %s -e entry -entry-point-result=void -print-mlir=mid 2>&1 | FileCheck %s --check-prefix=MID
// RUN: astl-run %s -e entry -entry-point-result=void -print-mlir=late  2>&1 | FileCheck %s --check-prefix=LATE
// RUN: astl-run %s -e entry -entry-point-result=void -print-mlir=llvm  2>&1 | FileCheck %s --check-prefix=LLVM

// Loops options (should be the same for both cases in this small example)
// RUN: astl-run %s -e entry -entry-point-result=void -print-mlir=late -astl-to-loops 2>&1 | FileCheck %s --check-prefix=LOOPS
// RUN: astl-run %s -e entry -entry-point-result=void -print-mlir=late -linalg-to-loops 2>&1 | FileCheck %s --check-prefix=LOOPS

// Benchmark options
// RUN: astl-run %s -e entry -entry-point-result=void -print -print-mlir=early 2>&1 | FileCheck %s --check-prefix=BENCH_PRINT
// RUN: astl-run %s -e entry -entry-point-result=void -n 10 -print-mlir=late 2>&1 | FileCheck %s --check-prefix=BENCH_STATS
// RUN: astl-run %s -e entry -entry-point-result=void -n 2 -print-mlir=late 2>&1 | FileCheck %s --check-prefix=BENCH_STATS_2
// RUN: astl-run %s -e entry -entry-point-result=void -n 100 -print-mlir=late 2>&1 | FileCheck %s --check-prefix=BENCH_STATS_100
// RUN: astl-run %s -e entry -entry-point-result=void -n 2000 -print-mlir=late 2>&1 | FileCheck %s --check-prefix=BENCH_STATS_2000

// CPU options can't be tested as even the LLVM IR is identical
// Splat and init options in astl-run-splat-* tests
