// RUN: astl-run %s -e entry -entry-point-result=void -print-mlir=early 2>&1 | \
// RUN: FileCheck %s --check-prefix=SPLAT
// RUN: astl-run %s -e entry -entry-point-result=void -print-mlir=early -seed 123 2>&1 | \
// RUN: FileCheck %s --check-prefix=RANDOM
// RUN: astl-run %s -e entry -entry-point-result=void -print-mlir=early -seed 123 -splat-to-random 2>&1 | \
// RUN: FileCheck %s --check-prefix=RANDOM-SPLAT

// Options for -init-type
// RUN: astl-run %s -e entry -entry-point-result=void -print-mlir=early -seed 123 -splat-to-random -init-type=const 2>&1 | \
// RUN: FileCheck %s --check-prefix=OPT-CONST
// RUN: astl-run %s -e entry -entry-point-result=void -print-mlir=early -seed 123 -splat-to-random -init-type=simple 2>&1 | \
// RUN: FileCheck %s --check-prefix=OPT-SIMPLE
// RUN: astl-run %s -e entry -entry-point-result=void -print-mlir=early -seed 123 -splat-to-random -init-type=cont 2>&1 | \
// RUN: FileCheck %s --check-prefix=OPT-CONT
// RUN: astl-run %s -e entry -entry-point-result=void -print-mlir=early -seed 123 -splat-to-random -init-type=random 2>&1 | \
// RUN: FileCheck %s --check-prefix=OPT-RANDOM
// RUN: astl-run %s -e entry -entry-point-result=void -print-mlir=early -seed 123 -splat-to-random -init-type=normal 2>&1 | \
