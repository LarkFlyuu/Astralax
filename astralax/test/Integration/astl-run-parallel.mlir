// RUN: astl-run %s -def-parallel=0 -e entry -entry-point-result=void -print-mlir=late -print 2>&1 | \
// RUN: FileCheck %s --check-prefix=SERIAL

// RUN: astl-run %s -def-parallel=1 -e entry -entry-point-result=void -print-mlir=late -print 2>&1 | \
