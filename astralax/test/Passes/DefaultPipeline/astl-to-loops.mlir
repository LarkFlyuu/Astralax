// RUN: astl-opt %s -default-astl-passes="astl-to-loops" -split-input-file | FileCheck %s

// CHECK-NOT: func.func private @xsmm_
// CHECK: func.func @astl_add(
// CHECK-SAME:  %[[ARG0:.+]]: memref<3x3xf32>,
// CHECK-SAME:  %[[ARG1:.+]]: memref<3x3xf32>
func.func @astl_add(%arg0: memref<3x3xf32>, %arg1: memref<3x3xf32>) {
  // CHECK: scf.for
  // CHECK:   scf.for
  // CHECK:     arith.addf
...
// CHECK-NOT: func.func private @xsmm_
// CHECK: func.func @astl_identity(
// CHECK-SAME:  %[[ARG0:.+]]: memref<3x3xf32>,
// CHECK-SAME:  %[[ARG1:.+]]: memref<1x1xf32>
func.func @astl_identity(%arg0: memref<3x3xf32>, %arg1: memref<1x1xf32>) {
  // CHECK: scf.for
  // CHECK:   scf.for
  // CHECK:     memref.load
  // CHECK:     memref.store
...
// CHECK-NOT: func.func private @xsmm_
// CHECK: func.func @astl_relu(
// CHECK-SAME:  %[[ARG0:.+]]: memref<3x3xf32>
func.func @astl_relu(%arg0: memref<3x3xf32>) {
  // CHECK: scf.for
  // CHECK:   scf.for
  // CHECK:     arith.maxf
