// RUN: astl-opt -pack-vnni -generalize-tensor-pack-unpack -bufferize %s | FileCheck %s

// RUN: astl-opt -pack-vnni %s | FileCheck %s -check-prefix=PACK
...
func.func @myfunc(%arg0: tensor<32x4x4xbf16>, %arg1: tensor<32x4x4xbf16>, %arg2: tensor<4x4xbf16>) -> tensor<4x4xbf16>{
  ...
  // CHECK: astl.brgemm ins(%{{.+}} : memref<32x4x4xbf16>, %{{.+}} : memref<32x2x4x2xbf16>, %{{.+}} : memref<4x4xbf16>) outs(%{{.+}} : memref<4x4xbf16>)
